...
build() {
  ...
  if [[ "${variant}" == "native" ]]; then
    ...
    if [[ "${run_mode}" == "run" ]]; then
      ...
      flatpak run --user com.vysp3r.Beamline
    fi
  fi
}
...
rebuild_translations() {
  ...
  ninja com.vysp3r.Beamline-update-po
...
flathub_linter() {
    ...
    BUILD_VARIANT="local"
    BUILD_MANIFEST="com.vysp3r.Beamline.local.yml"
    ...
    flatpak run --command=flatpak-builder-lint org.flatpak.Builder appstream "${BUILD_DIR}/export/share/metainfo/com.vysp3r.Beamline.metainfo.xml"
    ...
    show_log "INFO" "\"appid-filename-mismatch: com.vysp3r.Beamline.local\" (only happens on the local source code)"
...
main() {
  ...
  case "${1:-}" in
    local)
      build "local" "com.vysp3r.Beamline.local.yml" "${2:-}"
      ;;
    flathub)
      build "flathub" "com.vysp3r.Beamline.yml" "${2:-}"
      ;;
    native)
      build "native" "" "${2:-}"
      ;;
